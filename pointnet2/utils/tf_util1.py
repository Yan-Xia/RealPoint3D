...
def batch_norm_for_conv2d(inputs, is_training, bn_decay, scope):
  ...
  # return BatchNorm(inputs, decay = bn_decay, data_format = 'NHWC')
  # transpose NHWC to NCHW
  inputs = tf.transpose(inputs, perm=[0, 3, 1, 2])  
  # with tf.variable_scope(tf.get_default_graph().get_name_scope(), reuse=None):
  BN = BatchNorm('BatchNorm', inputs, decay = bn_decay, data_format = 'NCHW')
    # BN = BatchNorm('BatchNorm', inputs, data_format = 'NCHW')
    # BN = BatchNorm('BatchNorm', inputs)
  return tf.transpose(BN, perm = [0, 2, 3, 1])
